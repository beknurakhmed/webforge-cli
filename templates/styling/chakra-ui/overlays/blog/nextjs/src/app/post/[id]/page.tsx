  ...
  if (!post) {
    return (
      <Box textAlign="center" py={12} px={6}>
        <Heading as="h2" size="lg" color="gray.900" mb={2}>Post Not Found</Heading>
        <Text color="gray.500" mb={4}>The article you are looking for does not exist.</Text>
        <Link href="/" style={{ textDecoration: 'none' }}>
          <Box as="span" display="inline-block" bg="#4f46e5" color="white" px={6} py={2.5} borderRadius="lg" fontWeight={600} _hover={{ bg: '#4338ca' }}>Back to Blog</Box>
        </Link>
      </Box>
    );
  }
  ...
  return (
    <Box maxW="1100px" mx="auto" px={6} py={8}>
      <SimpleGrid columns={{ base: 1, lg: 2 }} gap={10} templateColumns={{ base: '1fr', lg: '1fr 320px' }} alignItems="start">
        <Box as="article">
          <Link href="/" style={{ textDecoration: 'none' }}>
            <Text display="inline-block" color="#4f46e5" fontWeight={500} mb={6} _hover={{ textDecoration: 'underline' }}>← Back to Blog</Text>
          </Link>
          <Box mb={8}>
            <Link href={`/category/${post.category.toLowerCase()}`} style={{ textDecoration: 'none' }}>
              <Text display="inline-block" fontSize="0.75rem" fontWeight={600} color="#4f46e5" textTransform="uppercase" letterSpacing="0.05em" bg="#eef2ff" px={2} py={0.5} borderRadius="sm">{post.category}</Text>
            </Link>
            <Heading as="h1" fontSize="2rem" fontWeight={800} color="gray.900" my={3} lineHeight={1.3}>{post.title}</Heading>
            <Flex align="center" gap={4} flexWrap="wrap">
              <Flex align="center" gap={2}>
                <Text fontSize="1.2rem">{post.authorAvatar}</Text>
                <Text fontWeight={600} color="gray.900" fontSize="0.85rem">{post.author}</Text>
              </Flex>
              <Text color="gray.500" fontSize="0.85rem">{post.date}</Text>
              <Text color="gray.500" fontSize="0.85rem">{post.readTime}</Text>
            </Flex>
          </Box>
          <Flex align="center" justify="center" fontSize="6rem" h="300px" bgGradient="linear-gradient(135deg, #eef2ff, #e0e7ff)" borderRadius="xl" mb={8}>
            {post.image}
          </Flex>
          <Box fontSize="1.05rem" lineHeight={1.8}>
            {post.content.split('\n\n').map((paragraph, i) => (
              <Text key={i} mb={5}>{paragraph}</Text>
            ))}
          </Box>
          <Flex gap={2} mt={8} pt={6} borderTop="1px solid" borderColor="gray.200">
            <Text bg="#eef2ff" color="#4f46e5" px={3} py={1} borderRadius="full" fontSize="0.8rem" fontWeight={500}>#{post.category.toLowerCase()}</Text>
            <Text bg="#eef2ff" color="#4f46e5" px={3} py={1} borderRadius="full" fontSize="0.8rem" fontWeight={500}>#webdev</Text>
            <Text bg="#eef2ff" color="#4f46e5" px={3} py={1} borderRadius="full" fontSize="0.8rem" fontWeight={500}>#programming</Text>
          </Flex>
        </Box>
        <BlogSidebar />
      </SimpleGrid>
