...
export default function CategoryPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8">
      <div className="mb-8">
        <Link href="/" className="inline-block text-indigo-600 font-medium mb-4 hover:underline">← All Posts</Link>
        <h1 className="text-2xl font-bold text-gray-900 mb-1">📂 {categoryName}</h1>
        <p className="text-gray-500">{filteredPosts.length} article{filteredPosts.length !== 1 ? 's' : ''} in this category</p>
      </div>
        ...
        <section>
          {filteredPosts.length > 0 ? (
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <Link href="/" className="inline-block px-6 py-2.5 rounded-lg font-semibold bg-indigo-600 text-white hover:bg-indigo-700 transition-colors">View All Posts</Link>
            </div>
          )}
        </section>
        <BlogSidebar />
