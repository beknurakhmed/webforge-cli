...
<template>
  <div v-if="post">
    <NuxtLink to="/" class="inline-block text-indigo-600 font-medium mb-6 hover:underline">← Back to Posts</NuxtLink>

    <article class="max-w-3xl mx-auto">
      <div class="bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-xl flex items-center justify-center p-12 mb-8">
        <span class="text-7xl">{{ post.image }}</span>
      </div>

      <div class="flex items-center gap-2 mb-3 text-sm text-gray-500">
        <NuxtLink :to="`/category/${post.category.toLowerCase()}`" class="text-indigo-600 font-semibold uppercase tracking-wider text-xs hover:underline">
          {{ post.category }}
        </NuxtLink>
      ...
      <div class="flex justify-between items-center pb-6 mb-8 border-b border-gray-200 text-sm">
        <span class="font-semibold text-gray-700">By {{ post.author }}</span>
        <span class="text-gray-400">{{ post.date }}</span>
      </div>

      <div>
        <p v-for="(paragraph, idx) in post.content.split('\n\n')" :key="idx" class="mb-6 text-lg leading-relaxed text-gray-700">
          {{ paragraph }}
        </p>
      </div>
    </article>
  ...
  <div v-else class="text-center py-16">
    <h2 class="text-gray-900 font-bold text-xl mb-4">Post not found</h2>
    <NuxtLink to="/" class="inline-block text-indigo-600 font-medium hover:underline">← Back to Posts</NuxtLink>
  </div>
</template>
